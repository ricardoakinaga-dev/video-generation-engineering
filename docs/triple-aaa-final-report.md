...
| Documentation checker | PASS; 46 docs, 50 YAML blocks, 495 local links, 80 requirements; current r21 evidence |
...
5. The resource guard is implemented and regression-tested, but it cannot create missing production evidence; R17 rejected promotion and a final frozen read-only review must follow the current evidence/accounting commit.
