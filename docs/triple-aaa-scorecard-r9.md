...
| 3 | Progressive Disclosure | 90 | Routing activates specialist references from concrete dialogue/audio, continuity, interaction, long-form, evaluation and observability signals, while keeping a simple scene on core guidance. | Context routing is structurally tested, not a user-outcome guarantee. |
...
| 11 | Model Adaptation | 75 | Profile freshness, canonical-state preservation, explicit loss and collected runtime/problem/artifact evidence for adapter differentials are enforced. | Only one real runtime is evidenced; second adapter is blocked. |
...
| 13 | Resource Safety | 92 | Read-only resource/queue snapshot and authorization/side-effect guards pass; queue state is explicitly non-authorizing. | No new generation was authorized in this run. |
| 14 | Deterministic QA | 96 | 161 tests, no-bytecode verification, static import-cycle audit, structural release checks and CLI exit semantics pass. | The separate release-assurance gate remains `INCOMPLETE`; determinism verifies software behavior, not model quality. |
...
| 18 | Repairability | 82 | Owner routing, bounded budgets, parent/new attempt lineage, diagnosis/owner/delta, chronological before/after proof and transition revalidation are enforced. | No real repair/re-anchor execution was authorized. |
...
| 20 | Portability | 96 | 30-file deterministic archive, external-CWD smoke, compile, local-link, full verifier and documentation checks pass. | Portable structure does not prove provider capability. |
...
| 22 | Production Evidence | 30 | Final release accounting preserves LF-001 partial evidence and explicit LF-002/LF-003/FLF/adapter blockers; the post-freeze reviewer matrix and exact scope binding are preserved in [`triple-aaa-independent-critic-current.md`](../.gauntlet/triple-aaa-independent-critic-current.md). | The overall verdict remains `PARTIAL`, not `TRIPLE_AAA_PROVEN`; independent reviewer `PASS` is still required. |

## Score interpretation

The software candidate is `READY_WITH_RISKS` for the scoped structural/local-runtime capability. The overall release remains `PARTIAL` until LF-001, repair, FLF, LF-002, LF-003, the second adapter (or its justified blocked record), an independent critic `PASS`, and the production gates satisfy the frozen bar. The scorecard intentionally does not average the remaining blockers away.
