...
| 8 | Dialogue/Performance | 45 | Five dialogue channels, speaker order and listener-mouthing constraints are separate. | No real dialogue performance or listening evidence exists. |
...
| 14 | Deterministic QA | 94 | 157 tests, no-bytecode verification, static import-cycle audit and CLI exit semantics pass. | Determinism verifies software behavior, not model quality. |
...
| 22 | Production Evidence | 35 | Fresh critic and final release accounting preserve LF-001 partial evidence and explicit LF-002/LF-003/FLF/adapter blockers. | The overall verdict remains `PARTIAL`, not `TRIPLE_AAA_PROVEN`. |

## Score interpretation

The software candidate is `READY_WITH_RISKS` for the scoped structural/local-runtime capability. The overall release remains `PARTIAL` until LF-001, repair, FLF, LF-002, LF-003, the second adapter (or its justified blocked record), and the production gates satisfy the frozen bar. The final independent review remains a separate release gate; the scorecard intentionally does not average the remaining production blockers away.
