...
Each `dialogue_timeline` item contains `id`, `shot_id`, `speaker`, `target_listener`, `text`, `start_s`, `end_s`, delivery/performance notes, start/end conditions and required reaction. Optional `reaction_at_s` precedes the end only with `intentional_overlap: true`. Both overlapping lines must explicitly declare overlap. Times are relative to the owning shot.
